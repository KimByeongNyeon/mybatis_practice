...
<link rel="icon"
	href="http://192.168.10.219/mybatis_prj/common/favicon.ico" />
<!--bootstrap 시작-->
<link
...
<!--bootstrap 끝-->
<link rel="stylesheet"
	href="http://192.168.10.219/mybatis_prj/common/css/main.css"
	type="text/css" media="all" />
<link rel="stylesheet"
	href="http://192.168.10.219/mybatis_prj/common/css/board.css"
	type="text/css" media="all" />
...
#header {
	height: 100px;
	background: #FFFFFF
		url('http://localhost/mybatis_prj/common/images/header.png') no-repeat;
}
	...
	$(function() {
		$("#btnList")
				.click(
						function() {
							//history.back();
							location.href = "http://192.168.10.219/mybatis_prj/board/board_list.jsp?currentPage=${ param.currentPage}";
						});//click
	...
	function searchReply() {
		...
		$
				.ajax({
					...
					success : function(jsonObj) {
						var output = "댓글을 읽어올 수 없습니다. 잠시 후 다시 시도해주세요.";
						if (jsonObj.flag) {
							output = "";
							...
							$
									.each(
											jsonObj.data,
											function(i, jsonTemp) {
												output += "<div><strong>내용</strong> : "
														+ jsonTemp.content
														+ "<br/>";
												output += "<strong>작성자 </strong> : "
														+ jsonTemp.writer
														+ "<strong>작성일</strong>"
														+ jsonTemp.input_date
														+ "<strong>ip</strong>"
														+ jsonTemp.ip
														+ "<input type='button' value='삭제' class='btn btn-warning btn-sm' onclick='removeReply("
														+ jsonTemp.num
														+ ")'>"
														+ "</div>";
											});//each
...
<body>
	<div id="wrap">
		...
		<div id="boardContent">
			...
			%>
			<script type="text/javascript">
				location.href = "http://192.168.10.219/mybatis_prj/error/err_500.html";
			</script>
			<%
			} //end catch
			%>
			...
			<form method="post" name="frmDetail" id="frmDetail">
				<input type="hidden" name="num" value="${ param.seq }" /> <input
					type="hidden" name="currentPage" value="${ param.currentPage }" />
				<table>
					<tr>
					...
					<tr>
						...
						<td><input type="text" name="title" id="title"
							style="width: 600px" value="${ bVO.title }" /></td>
					</tr>
					<tr>
						<td>내용</td>
						<td><textarea id="content" name="content">${bVO.content }</textarea>
						</td>
					</tr>
					<tr>
						<td>조회수</td>
						<td><input type="text" name="cnt" id="cnt"
							value="${ bVO.cnt }" style="width: 600px" /></td>
					</tr>
					<tr>
						<td>작성일</td>
						<td><strong><fmt:formatDate
									value="${ bVO.input_date }" pattern="yyyy-MM-dd EEEE HH:mm:ss" /></strong></td>
					</tr>
					<tr>
						<td>작성자</td>
					...
					<tr>
						<td colspan="2" style="text-align: center;">
								<input type="button" value="글수정" class="btn btn-success btn-sm"
									id="btnUpdate" />
								<input type="button" value="글삭제" class="btn btn-warning btn-sm"
									id="btnDelete" />
							 <input type="button" value="글목록" class="btn btn-info btn-sm"
							id="btnList" /></td>
					</tr>
				</table>
			</form>
		...
		<div id="reply_wrap">
			<div id="reply_frm">
				...
				내용<input type="text" name="rContent" id="rContent"
					style="width: 400px" /> 작성자<input type="text" name="rWriter"
					id="rWriter" style="width: 100px" /> <input type="button"
					value="댓글작성" id="btnWrite" class="btn btn-success btn-sm" />
			</div>
